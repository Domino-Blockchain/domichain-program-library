...
the desired address to send the token account's satomis.
...
import {clusterApiUrl, Connection, Keypair, SATOMIS_PER_SOL, SystemProgram, Transaction, sendAndConfirmTransaction} from "@solana/web3.js";
...
console.log(`Native: ${accountInfo.isNative}, Satomis: ${accountInfo.amount}`);
...
*Note*: Some satomis were removed for transaction fees
...
Fee: 5000 satomis per signature
