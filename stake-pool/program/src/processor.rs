...
impl Processor {
    ...
    fn process_add_validator_to_pool(
        ...
        validator_list.push(ValidatorStakeInfo {
            ...
            active_stake_satomis: required_satomis.into(),
    ...
    fn process_remove_validator_from_pool(
        ...
        let new_status = if u64::from(validator_stake_info.transient_stake_satomis) > 0 {
            check_transient_stake_address(
                program_id,
                stake_pool_info.key,
    ...
    fn process_decrease_validator_stake(
        ...
        if u64::from(validator_stake_info.transient_stake_satomis) > 0 {
        ...
        let stake_minimum_delegation = stake::tools::get_minimum_delegation()?;
        let current_minimum_satomis = minimum_delegation(stake_minimum_delegation);
        ...
        let (source_stake_account_info, split_satomis) =
                ...
                maybe_ephemeral_stake_seed.zip(maybe_ephemeral_stake_account_info)
            {
                ...
                if let Some(reserve_stake_info) = maybe_reserve_stake_info {
                    let required_satomis_for_rent_exemption =
                        stake_rent.saturating_sub(ephemeral_stake_account_info.satomis());
                    if required_satomis_for_rent_exemption > 0 {
                        if required_satomis_for_rent_exemption >= reserve_stake_info.satomis() {
                        ...
                        Self::stake_withdraw(
                            ...
                            clock_info.clone(),
                            stake_history_info.clone(),
                            required_satomis_for_rent_exemption,
                        )?;
                    }
                }
        ...
        validator_stake_info.active_stake_satomis =
            u64::from(validator_stake_info.active_stake_satomis)
                .checked_sub(satomis)
                .ok_or(StakePoolError::CalculationFailure)?
                .into();
        validator_stake_info.transient_stake_satomis =
            transient_stake_account_info.satomis().into();
    ...
    fn process_increase_validator_stake(
            ...
            < stake_rent
        {
            ...
            msg!(
                "Reserve stake does not have enough satomis for increase, maximum amount {}, {} requested",
                max_split_amount,
                satomis
            );
            return Err(ProgramError::InsufficientFunds);
        ...
        validator_stake_info.transient_stake_satomis =
            u64::from(validator_stake_info.transient_stake_satomis)
    ...
    fn process_redelegate(
        ...
        let vote_account_address = stake.delegation.voter_pubkey;
        {
            ...
            validator_stake_info.active_stake_satomis =
                u64::from(validator_stake_info.active_stake_satomis)
                    ...
                    .ok_or(StakePoolError::CalculationFailure)?
                    .into();
            validator_stake_info.transient_stake_satomis = stake_rent.into();
        ...
        // split from source, into source transient
        {
            ...
            let required_satomis_for_rent_exemption =
                stake_rent.saturating_sub(source_transient_stake_account_info.satomis());
            if required_satomis_for_rent_exemption > 0 {
                if required_satomis_for_rent_exemption >= reserve_stake_info.satomis() {
                    return Err(StakePoolError::ReserveDepleted.into());
                }
...

        {
            ...
            let transient_account_exists =
                u64::from(validator_stake_info.transient_stake_satomis) > 0;
            validator_stake_info.transient_stake_satomis =
                u64::from(validator_stake_info.transient_stake_satomis)
                    .checked_add(satomis)
                    .ok_or(StakePoolError::CalculationFailure)?
    ...
    fn process_update_validator_list_balance(
        ...
        for (validator_stake_record, validator_stakes) in validator_iter {
            ...
            validator_stake_record.active_stake_satomis = active_stake_satomis.into();
            validator_stake_record.transient_stake_satomis = transient_stake_satomis.into();
        }
    ...
    fn process_deposit_stake(
        ...
        validator_stake_info.active_stake_satomis = validator_stake_account_info.satomis().into();
        ...
        let validator_list_item_info = if *stake_split_from.key == stake_pool.reserve_stake {
                ...
                stake_pool.preferred_withdraw_validator_vote_address
            {
                let preferred_validator_info = validator_list
                    ...
                    .ok_or(StakePoolError::ValidatorNotFound)?;
                let available_satomis = u64::from(preferred_validator_info.active_stake_satomis)
                    .saturating_sub(minimum_satomis_with_tolerance);
                if preferred_withdraw_validator != vote_account_address && available_satomis > 0 {
                    msg!("Validator vote address {} is preferred for withdrawals, it currently has {} satomis available. Please withdraw those before using other validator stake accounts.", preferred_withdraw_validator, u64::from(preferred_validator_info.active_stake_satomis));
            ...
            match withdraw_source {
                StakeWithdrawSource::Active => {
                    validator_list_item.active_stake_satomis =
                        u64::from(validator_list_item.active_stake_satomis)
                            .checked_sub(withdraw_satomis)
                            .ok_or(StakePoolError::CalculationFailure)?
                            .into()
                }
                StakeWithdrawSource::Transient => {
                    validator_list_item.transient_stake_satomis =
                        u64::from(validator_list_item.transient_stake_satomis)
                            .checked_sub(withdraw_satomis)
                            .ok_or(StakePoolError::CalculationFailure)?
                            .into()
                }
                StakeWithdrawSource::ValidatorRemoval => {
                    validator_list_item.active_stake_satomis =
                        u64::from(validator_list_item.active_stake_satomis)
                            ...
                            .ok_or(StakePoolError::CalculationFailure)?
                            .into();
                    if u64::from(validator_list_item.active_stake_satomis) != 0 {
...
impl PrintProgramError for StakePoolError {
        ...
        E: 'static + std::error::Error + DecodeError<E> + PrintProgramError + FromPrimitive,
    {
        match self {
            ...
            StakePoolError::ReserveDepleted => msg!("Error: Pool reserve does not have enough satomis to fund rent-exempt reserve in split destination. Deposit more SOL in reserve, or pre-fund split destination with the rent-exempt reserve for a stake account."),
