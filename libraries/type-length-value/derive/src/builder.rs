    ...
    fn from(builder: &SplBorshVariableLenPackBuilder) -> Self {
        ...
        quote! {
            impl #generics spl_type_length_value::variable_len_pack::VariableLenPack for #ident #generics #where_clause {
                fn pack_into_slice(&self, dst: &mut [u8]) -> Result<(), domichain_program::program_error::ProgramError> {
                    borsh::to_writer(&mut dst[..], self).map_err(Into::into)
                }

                fn unpack_from_slice(src: &[u8]) -> Result<Self, domichain_program::program_error::ProgramError> {
                    domichain_program::borsh::try_from_slice_unchecked(src).map_err(Into::into)
                }

                fn get_packed_len(&self) -> Result<usize, domichain_program::program_error::ProgramError> {
                    domichain_program::borsh::get_instance_packed_len(self).map_err(Into::into)
                }
            }
        }
    }
