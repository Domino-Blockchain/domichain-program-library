...
/// Builds the implementation of `Into<domichain_program::program_error::ProgramError>`
/// More specifically, implements `From<Self> for domichain_program::program_error::ProgramError`
pub fn into_program_error(ident: &Ident) -> proc_macro2::TokenStream {
    quote! {
        impl From<#ident> for domichain_program::program_error::ProgramError {
...
/// Builds the implementation of `domichain_program::decode_error::DecodeError<T>`
pub fn decode_error(ident: &Ident) -> proc_macro2::TokenStream {
    quote! {
        impl<T> domichain_program::decode_error::DecodeError<T> for #ident {
...
/// Builds the implementation of `domichain_program::program_error::PrintProgramError`
pub fn print_program_error(
    ...
    let ppe_match_arms = variants.iter().map(|variant| {
        ...
        quote! {
            #ident::#variant_ident => {
                domichain_program::msg!(#error_msg)
            }
        }
    });
    quote! {
        impl domichain_program::program_error::PrintProgramError for #ident {
            fn print<E>(&self)
            where
                E: 'static
                    + std::error::Error
