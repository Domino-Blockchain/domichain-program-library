...
use domichain_cli_output::OutputFormat;
...
impl<'a> Config<'a> {
    pub(crate) async fn new(
        ...
        let cli_config = if let Some(config_file) = matches.value_of("config_file") {
            domichain_cli_config::Config::load(config_file).unwrap_or_else(|_| {
                eprintln!("error: Could not find config file `{}`", config_file);
                exit(1);
            })
        } else if let Some(config_file) = &*domichain_cli_config::CONFIG_FILE {
        ...
        let websocket_url = domichain_cli_config::Config::compute_websocket_url(&json_rpc_url);
    ...
    pub(crate) fn default_signer(&self) -> Result<Arc<dyn Signer>, Error> {
        if let Some(default_signer) = &self.default_signer {
            ...
            Err("default signer is required, please specify a valid default signer by identifying a \
                 ...
                 config at the default location of ~/.config/domichain/cli/config.yml using the domichain \
    ...
    pub(crate) fn fee_payer(&self) -> Result<Arc<dyn Signer>, Error> {
        if let Some(fee_payer) = &self.fee_payer {
            ...
            Err("fee payer is required, please specify a valid fee payer using the --fee_payer argument, \
                 ...
                 creating a valid config at the default location of ~/.config/domichain/cli/config.yml using \
                 the domichain config command".to_string().into())
        }
    }
