...
fn process_deposit(
    ...
    // Wrapped SOL deposits are not supported because satomis cannot be vanished.
...
fn process_withdraw(
    ...
    // Wrapped SOL withdrawals are not supported because satomis cannot be apparated.
    assert!(!token_account.base.is_native());
